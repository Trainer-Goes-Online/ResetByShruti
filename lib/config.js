...
export const CONFIG = {
  ...
  /* ---- LEGAL ENTITY ----------------------------------------------------
     Razorpay's merchant verification cross-checks the name and address on your
     policy pages against the entity on the account. These are facts about a
     real business — they are never guessed. Each renders a visible [gap] chip
     until supplied. See QUESTIONS-FOR-CLIENT.txt Q20.3 / QL.1–QL.6. */
  LEGAL_ENTITY: or(process.env.NEXT_PUBLIC_LEGAL_ENTITY),        // QL.1
  BUSINESS_ADDRESS: or(process.env.NEXT_PUBLIC_BUSINESS_ADDRESS), // QL.2
  SUPPORT_PHONE: or(process.env.NEXT_PUBLIC_SUPPORT_PHONE),       // QL.3
  GRIEVANCE_OFFICER: or(process.env.NEXT_PUBLIC_GRIEVANCE_OFFICER), // QL.4
  JURISDICTION: or(process.env.NEXT_PUBLIC_JURISDICTION),         // QL.5 — city whose courts govern
  GSTIN: or(process.env.NEXT_PUBLIC_GSTIN),                       // QL.6 — omit if unregistered
  POLICY_UPDATED: or(process.env.NEXT_PUBLIC_POLICY_UPDATED, '23 July 2026'),
